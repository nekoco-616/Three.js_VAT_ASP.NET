// wwwroot/js/fluid-vat-viewer.js
...
(function () {
    ...
    window.initFluidVatViewer = async (canvasId) => {
        ...
        const [fbx, posTexture] = await Promise.all([
            new THREE.FBXLoader().loadAsync("models/Fluid.fbx"),
            new THREE.EXRLoader().loadAsync("textures/Fluid_pos.exr")
        ]);
        ...
        const model = fbx;
        scene.userData.fluidModel = model;
    ...
    window.setFluidVatViewerMaterial = (name) => {
        if (scene && scene.userData.fluidModel && scene.userData.materials) {
            const material = scene.userData.materials[name];
            if (material) {
                scene.userData.fluidModel.traverse((child) => {
                    if (child.isMesh) {
