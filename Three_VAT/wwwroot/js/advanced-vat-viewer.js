(function() {
    ...
    window.initAdvancedVatViewer = (containerSelector) => {
        ...
        const animate = () => {
            ...
            if (scene.userData.shaderMaterial) {
                scene.userData.shaderMaterial.uniforms.time.value = time;
            }
    ...
    async function loadAssets() {
        ...
        try {
            const [modelObject, posTexture, colTex] = await Promise.all([
                fbxLoader.loadAsync('/models/g15_b5.fbx'),
                exrLoader.loadAsync('/textures/g15_b5_pos_diff.exr'),
                textureLoader.loadAsync('/textures/g15_b5_volume.png')
            ]);
            ...
            const uniforms = {
                ...
                // The original shader used _MainTex for color, which seems to be the same as the position texture's alpha in some cases.
                // Here we'll use the position texture itself for the color data, assuming it's stored there.
                colorTex: { value: colTex }, 
                time: { value: 0.0 },
                timeMultiplier: { value: 10.0 },
                maxFrame: { value: 240 }, // Number of animation frames
                texelSizeY: { value: 1.0 / texHeight },
                quadSize: { value: 0.15 },
                minSize: { value: 0.4 },
                height: { value: 1.0 }, // Flutter amount
                isLerp: { value: true },
            ...
            const shaderMaterial = new THREE.ShaderMaterial({
                ...
                transparent: true,
                defines: {
                    // For any preprocessor directives if needed
                }
            });

            scene.userData.shaderMaterial = shaderMaterial;

            modelObject.traverse((child) => {
            if (child.isMesh) {
                console.log("--- DEBUG: Geometry Attributes ---");
    ...
    window.updateTimeMultiplier = (value) => {
        if (scene && scene.userData.shaderMaterial) {
            scene.userData.shaderMaterial.uniforms.timeMultiplier.value = value;
        }
    };

    window.updateMinSize = (value) => {
        if (scene && scene.userData.shaderMaterial) {
            scene.userData.shaderMaterial.uniforms.minSize.value = value;
        }
    };
    ...
    window.updatePassiveCol = (alpha) => {
        if (scene && scene.userData.shaderMaterial) {
            ...
            scene.userData.shaderMaterial.uniforms.passiveCol.value.set(color.r, color.g, color.b, alpha);
        }
    };
